...
def find_direction(z, y, Classifier, f=5, max_iter=1000):
    ...
    while i < max_iter:
        ...
        # residual = -torch.autograd.grad(loss, z, create_graph=True)[0]
        # reg_loss = lambda_reg * torch.sum(torch.pow(z, 2))
        # reg_grad = torch.autograd.grad(reg_loss, z)[0]
        # z = z - f * (residual + reg_grad)
        z = z + f * residual
        z = z.float()
        i += 1
